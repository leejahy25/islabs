...
```
openssl enc -aes-256-cbc -salt -in file.txt -out file.txt.enc -pass file:symmetric.key
```
...
```
tar -cf - file.txt.enc symmetric.key.enc | nc -l -p 5678
```
...
```
openssl enc -aes-256-cbc -d -in file.txt.enc -out file.txt -pass file:symmetric.key
```
